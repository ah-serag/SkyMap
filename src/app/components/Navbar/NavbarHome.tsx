...
const links = [
  ...
  { href: "/About", label: "About", icon: faDesktop },
...
const NavbarHome: React.FC = () => {
  ...
  const isActive = (path: string) =>
    pathname === path ? "border-b-accent  border-b-1" : "";
  const toggleMobile = () => setIsMobileOpen(!isMobileOpen);
  const { resolvedTheme } = useTheme();

  return (
    <nav className="w-full fixed  h-[59px] z-50 border-b-1 border-accent top-0 bg-card text-foreground">
      <div className="py-1 h-full  px-2 mx-auto bg-card flex justify-between items-center">
        {/* Logo */}
